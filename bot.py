...
EMOJI_MAP = [
    (["ночь", "ночная", "3 ночи", "ночью"], "🌙"),
    (["утро", "утром", "просну", "рассвет"], "🌅"),
    (["дождь", "дождл", "ливень"], "🌧️"),
    (["злой", "злая", "бесит", "бесят", "агресс"], "😤"),
    (["грустн", "слезы", "слёзы", "плакать", "печаль"], "😭"),
    (["весел", "смех", "смешн", "прикол"], "😂"),
    (["стыдн", "стыд", "позор"], "😳"),
    (["энерги", "заряжа", "бодр"], "⚡"),
    (["машин", "поездк", "дорог", "авто"], "🚗"),
    (["лето", "летн", "пляж", "жара"], "🌴"),
    (["зима", "зимн", "снег", "холод"], "❄️"),
    (["осень", "осенн"], "🍂"),
    (["весна", "весенн"], "🌱"),
    (["победа", "чемпион", "герой", "главный"], "🏆"),
    (["спорт", "трениров", "бег", "качал"], "🏋️"),
    (["любов", "влюб", "сердц", "романт"], "❤️"),
    (["пьянк", "вписк", "пиво", "алкоголь"], "🍺"),
    (["детств", "школ", "14 лет", "молодост"], "📻"),
    (["страшн", "ужас", "жутк", "тьма"], "💀"),
    (["космос", "галактик", "вселенн"], "🚀"),
    (["город", "улиц", "метро"], "🏙️"),
    (["природ", "лес", "горы", "поле"], "🌾"),
    (["работ", "учеб", "концентр", "думать"], "🧠"),
    (["тайн", "секрет", "никто не знает"], "🤫"),
    (["танц", "клуб", "вечеринк"], "🎉"),
    (["тяжел", "метал", "рок", "хард"], "🤘"),
    (["нежн", "тих", "спокойн", "расслаб"], "🌸"),
    (["сон", "засыпа", "перед сном", "спать"], "💤"),
    (["репит", "по кругу", "снова и снова"], "🔁"),
    (["орать", "кричать", "громк"], "🔊"),
    (["игра", "игры", "геймер", "видеоигр"], "🎮"),
    (["кино", "фильм", "саундтрек"], "🎬"),
    (["ностальг", "воспомина", "прошлое"], "🥲"),
    (["чугун", "железо", "сталь", "молот"], "🔨"),
    (["деньги", "богатств", "понты"], "💰"),
    (["странн", "безумн", "сумасш"], "🛸"),
    (["наушник", "детали", "внимательн"], "🔬"),
    (["открыти", "изменил", "открыл"], "🦋"),
    (["недооцен", "мало знают"], "💎"),
    (["завируси", "хайп", "популярн"], "📱"),
]
...
def add_theme_to_db(theme_text):
    conn = sqlite3.connect("trackday.db")
    c = conn.cursor()
    now = datetime.now().isoformat()
    try:
        c.execute("INSERT INTO themes (theme, used, added_at) VALUES (?, 0, ?)", (theme_text, now))
        conn.commit()
        result = True
...
async def themes_list(callback: CallbackQuery):
    ...
    text = f"📋 <b>Темы (страница {page+1}/{total_pages}):</b>\n\n"
    for theme_id, theme_text, used in page_themes:
        status = "✅" if not used else "☑️"
        text += f"{status} <b>#{theme_id}</b> {theme_text}\n\n"
...
async def handle_private_message(message: Message):
    ...
    if admin_states.get(user_id) == "waiting_theme" and user_id == ADMIN_ID:
        ...
        if add_theme_to_db(theme_with_emoji):
            total, unused = get_themes_count()
            await message.answer(
    ...
    if admin_states.get(user_id) == "waiting_delete_id" and user_id == ADMIN_ID:
        try:
            theme_id = int(message.text.strip())
            conn = sqlite3.connect("trackday.db")
            c = conn.cursor()
            ...
            if theme:
                ...
                await message.answer("❌ Тема с таким номером не найдена. Попробуй ещё раз или /cancel")
        except ValueError:
            await message.answer("❌ Введи число — номер темы.")
        return
